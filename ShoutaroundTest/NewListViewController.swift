...
protocol NewListViewControllerDelegate {
...
//            if currentDisplayList?.postIds!.count > 0 {
//                var earliestPostID = ""
//                var earliestPostTime
//                for post in currentDisplayList?.postIds {
//
//                }
//            }
            
            
//            if let image = currentDisplayList?.listImageUrls[0] {
//                self.heroBackgroundImageView.loadImage(urlString: image)
//            } else {
//                self.heroBackgroundImageView.image = UIImage()
//            }
...
//    }()
    ...
    let listSummaryLabel: UILabel = {
        ...
        //        label.textColor = UIColor.init(hexColor: "F1C40F")
        label.text = "Some of the best experience I've had finding food in Malaysia"
        label.textAlignment = NSTextAlignment.left
        label.backgroundColor = UIColor.clear
...
//        button.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(listSettingButtonPressed)))
    ...
    func listSettingButtonPressed(){
        ...
        if currentDisplayList?.creatorUID != Auth.auth().currentUser?.uid {
            ...
            self.alert(title: "List Setting", message: "List Setting")
        }
...
//        button.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(listEditButtonPressed)))
    ...
    override func viewDidLoad() {
        ...
        heroBackgroundImageView.anchor(top: view.topAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 280)
        
        
    // SETUP LIST DETAIL VIEW
        view.addSubview(listDetailView)
        listDetailView.backgroundColor = UIColor.white
        listDetailView.anchor(top: heroBackgroundImageView.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, paddingTop: -20, paddingLeft: 15, paddingBottom: 0, paddingRight: 15, width: 0, height: 100)
        ...
        view.addSubview(listDetailDivider)
        listDetailDivider.backgroundColor = UIColor.lightGray
        listDetailDivider.anchor(top: nil, left: listDetailView.leftAnchor, bottom: nil, right: listDetailView.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 0.5)
        listDetailDivider.centerYAnchor.constraint(equalTo: listDetailView.centerYAnchor).isActive = true
        
        
        view.addSubview(listSummaryLabel)
        listSummaryLabel.anchor(top: listDetailView.topAnchor, left: listDetailView.leftAnchor, bottom: listDetailDivider.topAnchor, right: listDetailView.rightAnchor, paddingTop: 5, paddingLeft: 15, paddingBottom: 5, paddingRight: 15, width: 0, height: 0)
        
        let listActionBar = UIView()
        view.addSubview(listActionBar)
        ...
        followerCountLabel.anchor(top: nil, left: listActionBar.leftAnchor, bottom: nil, right: nil, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 0)
        followerCountLabel.centerYAnchor.constraint(equalTo: listActionBar.centerYAnchor).isActive = true
        
        view.addSubview(listFollowEditButton)
        listFollowEditButton.anchor(top: listActionBar.topAnchor, left: nil, bottom: listActionBar.bottomAnchor, right: listActionBar.rightAnchor, paddingTop: 5, paddingLeft: 5, paddingBottom: 5, paddingRight: 5, width: 0, height: 0)
        listFollowEditButton.sizeToFit()
        
        view.addSubview(listShareButton)
        listShareButton.anchor(top: listActionBar.topAnchor, left: nil, bottom: listActionBar.bottomAnchor, right: listFollowEditButton.leftAnchor, paddingTop: 5, paddingLeft: 5, paddingBottom: 5, paddingRight: 15, width: 0, height: 0)
        listShareButton.sizeToFit()
        
    // SETUP LIST NAME
        
        view.addSubview(listNameLabel)
        listNameLabel.anchor(top: nil, left: listDetailView.leftAnchor, bottom: listDetailView.topAnchor, right: listDetailView.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 20, paddingRight: 0, width: 0, height: 0)
        listNameLabel.heightAnchor.constraint(lessThanOrEqualToConstant: 1130).isActive = true
        
        
        view.addSubview(listSettingButton)
        listSettingButton.anchor(top: nil, left: listNameLabel.leftAnchor, bottom: listNameLabel.topAnchor, right: nil, paddingTop: 0, paddingLeft: 0, paddingBottom: 5, paddingRight: 0, width: 0, height: 25)
        listSettingButton.sizeToFit()
        setupListSettingButton()
        
        setupCollectionView()
        view.addSubview(imageCollectionView)
        imageCollectionView.anchor(top: listDetailView.bottomAnchor, left: view.leftAnchor, bottom: bottomLayoutGuide.topAnchor, right: view.rightAnchor, paddingTop: 0, paddingLeft: 15, paddingBottom: 0, paddingRight: 15, width: 0, height: 0)
...
//        setupEmojiDetailLabel()
    ...
    func setupListSettingButton() {
        ...
        if currentDisplayList?.creatorUID == Auth.auth().currentUser?.uid {
            ...
            let listSettingTitle = NSAttributedString(string: "  List Options", attributes: [NSAttributedString.Key.foregroundColor: UIColor.white, NSAttributedString.Key.font: UIFont(name: "Poppins-Bold", size: 12)])
            listString.append(listSettingTitle)
            
            let inputImage = #imageLiteral(resourceName: "settings_white").resizeVI(newSize: CGSize(width: 15, height: 15))!.withRenderingMode(.alwaysTemplate)
...
//        listShareButton.seta = shareButtonString
        ...
        if currentDisplayList?.creatorUID == Auth.auth().currentUser?.uid {
            ...
            let inputImage = #imageLiteral(resourceName: "settings_white").withRenderingMode(.alwaysTemplate)
            listFollowEditButton.setImage(inputImage, for: .normal)
            
            let listSettingTitle = NSAttributedString(string: " EDIT LIST", attributes: [NSAttributedString.Key.foregroundColor: UIColor.lightGray, NSAttributedString.Key.font: UIFont(name: "Poppins-Bold", size: 12)])
            listString.append(listSettingTitle)
...
//            listFollowEditButton.setAttributedTitle(listString, for: .normal)
        ...
        else
        {
            ...
            let followEditColor = isFollowing ? UIColor.ianLegitColor() : UIColor.white
            let followEditText = isFollowing ? " FOLLOWING" : " FOLLOW LIST"

            // NOT CURRENT USER - GOLLOW
            let image1Attachment = NSTextAttachment()
            let inputImage = isFollowing ? #imageLiteral(resourceName: "photo_upload_plus").withRenderingMode(.alwaysTemplate) : #imageLiteral(resourceName: "photo_upload_check").withRenderingMode(.alwaysOriginal)

            
            let listSettingTitle = NSAttributedString(string: followEditText, attributes: [NSAttributedString.Key.foregroundColor: followEditColor, NSAttributedString.Key.font: UIFont(name: "Poppins-Bold", size: 12)])
            listString.append(listSettingTitle)
...
//            listFollowEditButton.textColor = followEditColor
    ...
    func setupNavigationItems(){
        ...
        tempNavBarButton.layer.borderWidth = 0.5
        tempNavBarButton.layer.borderColor = UIColor.white.cgColor
        tempNavBarButton.setTitleColor(UIColor.white, for: .normal)
        tempNavBarButton.contentEdgeInsets = UIEdgeInsets(top: 5, left: 5, bottom: 5, right: 5)
    ...
    func filterSortFetchedPosts(){
        ...
        Database.sortPosts(inputPosts: self.fetchedPosts, selectedSort: listSort, selectedLocation: self.viewFilter?.filterLocation, completion: { (sortedPosts) in
            ...
            let earlyPostURL = self.fetchedPosts[0].imageUrls[0]
            if earlyPostURL != nil {
                self.heroBackgroundImageView.loadImage(urlString: earlyPostURL)
            }
