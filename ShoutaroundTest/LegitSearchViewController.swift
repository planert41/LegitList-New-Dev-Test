...
class LegitSearchViewController: UIViewController {
    ...
    var selectedSearchSegmentIndex: Int = 0
...
//        self.setupSearch()
        ...
        self.selectedSearchSegmentIndex = 0
...
extension LegitSearchViewController: UISearchBarDelegate {

    func setupSearch() {
        print("Setup Search | LegitSearchViewController")
    // SEARCH SEGMENT CONTROL
        ...
        searchSegment = UISegmentedControl(items: LegitSearchBarOptions)
        searchSegment.backgroundColor = UIColor.clear
        searchSegment.addTarget(self, action:
            #selector(LegitSearchViewController.searchSegmentChange), for: .valueChanged)
...
//        textFieldInsideUISearchBar?.backgroundColor = UIColor.ianWhiteColor()
    ...
    @objc func searchSegmentChange(sender: UISegmentedControl) {
        ...
        for (index, sortOptions) in LegitSearchBarOptions.enumerated()
        {
            ...
            sender.setWidth((isSelected) ? selectedSegmentWidth : unselectedSegmentWidth, forSegmentAt: index)
//            (sender.subviews[index] as! UIView).tintColor = isSelected ? UIColor.ianLegitColor() : UIColor.ianWhiteColor()
...
extension LegitSearchViewController: UICollectionViewDelegate, UICollectionViewDataSource, SelectedFilterBarCellDelegate {
...
extension LegitSearchViewController: UITableViewDataSource, UITableViewDelegate, EmptyDataSetSource, EmptyDataSetDelegate {
    ...
    // FOOD EMOJIS
        ...
        Database.sortEmojisWithCounts(inputEmojis: allEmojis, emojiCounts: foodEmojiCount, defaultCounts: defaultCounts, dictionaryMatch: false, sort: true) { (foodEmojis) in
